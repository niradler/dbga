"""CLI surface for the plural ``dbga sessions`` (list / cleanup).

Distinct from the singular ``session`` command set, which targets a single
named daemon. ``sessions ls`` scans ``.debug-agent/sessions/`` and:
...
from debug_agent.core.state import is_pid_alive, state_dir
