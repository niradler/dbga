...
Sometimes you're not debugging solo — you're pairing with a human who wants to drive from VS Code, or you want a richer UI than JSON-over-CLI. `debug-cli` supports two collab patterns:

1. **Shared breakpoints file** — both CLI and VS Code consume `.debug-cli/breakpoints.json`
2. **Listen mode** (`--listen PORT`) — `debug-cli` spawns the debuggee with debugpy listening; VS Code attaches
...
When you pass `--use-bps-file` on `session start`, the daemon merges `.debug-cli/breakpoints.json` into the initial breakpoint set. Any `set-bp` / `clear-bp` during the session is written back to the same file (unless you pass `--no-write-bps-file`).

```json
// .debug-cli/breakpoints.json
{
  "breakpoints": [
    {"file": "app.py", "line": 42},
...
- Have one teammate maintain `.debug-cli/breakpoints.json` and have the other sync VS Code breakpoints to match, or
...
1. Reads `.debug-cli/breakpoints.json` (or runs a sync script that copies entries into VS Code's bp list).
...
```json
{
  "type": "debugpy",
  "request": "attach",
  "name": "Attach to debug-cli session",
