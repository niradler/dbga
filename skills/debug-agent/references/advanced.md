...
dbga session eval --expr "i += 1; i"      # NOTE: side-effectful — only use to confirm the theory
...
```powershell
dbga instrument add lock_helper.py:20 --kind log \
...
```powershell
dbga session start --break-at "app.py:45:i == 5000" -- proc.py
# → stopped at i=5000
dbga session eval --expr "is_valid(state)"
# → True  → bug is after iteration 5000

dbga session set-bp "app.py:45:i == 7500"
dbga session clear-bp app.py:45         # the i==5000 one
...
```powershell
dbga session set-bp parser.py:exit
dbga session set-bp validator.py:entry
...
```powershell
dbga session eval --expr "data['nested']['list'][3]['payload']"
dbga session eval --expr "json.dumps(data, default=str, indent=2)[:2000]"
```
...
```powershell
dbga session eval --expr "df.head(20).to_dict('records')"
dbga session eval --expr "arr.shape, arr.dtype, arr.ravel()[:50].tolist()"
```
...
- If you genuinely need a native stack trace, run under `gdb python` or `lldb python` outside `dbga` — that's beyond Python-level debugging
...
3. **Instrument the child's entry point** to set up a `breakpoint()` that fires only under a specific env var:
   ```powershell
   dbga instrument add child.py:1 --kind breakpoint \
...
2. **Instrument with a conditional probe** that dumps state on the failing path:
   ```powershell
   dbga instrument add test_foo.py:42 --kind log \
...
Outside the scope of this skill — use `tracemalloc`, `objgraph`, or `memray` directly. `dbga session` can pause for inspection but doesn't summarize allocation patterns.
