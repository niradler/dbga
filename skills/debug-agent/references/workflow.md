...
```powershell
dbga run --timeout 30 -- python my_app.py --flag
```
...
```powershell
dbga localize --file traceback.txt --context-lines 5
```
...
```powershell
dbga session eval --expr "transform(items) if items else None"
```
...
dbga session continue       # to the same breakpoint
```
...
```powershell
# 1. Observe — reproduce
dbga run --timeout 5 -- python app_smoke.py
# → stdout: "None"
...
dbga session start --break-at app.py:41 -- app.py
...
dbga session continue --break app.py:8 --remove-break app.py:41
# → stopped at app.py:8, config_path="/tmp/empty.yaml" — file is empty

# 7. Verify the fix
dbga session eval --expr "load_config('/tmp/empty.yaml') or default_config()"
# → {...sensible defaults...}
